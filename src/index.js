...
module.exports = function () {
    return {
        ...
        async reportTaskStart (startTime, userAgents, testCount) {
            ...
            environments.push('Drupal_Staging');
        ...
        async reportTestDone (name, testRunInfo, meta) {
            ...
            // else
            //     currentTest.testKey = '';
            currentTest.testKey = meta.testCase ? meta.testCase : '';

            if (!testRunInfo.skipped) {
                if (hasErrors) {
                    testStatus = 'FAILED';

                    currentTest.evidence = [];

                    let errorData = '';
                    errors.forEach((error, idx) => {
                        errorData += this.formatError(error, `${idx + 1}) `);
                        errorData += '\n\n';
                    });

                    const buff = new Buffer(errorData);
                    currentEvidences.data = buff.toString('base64');
                    currentEvidences.filename = 'failure-log.txt';
                    currentEvidences.contentType = 'text/plain';
                    currentTest.evidence.push(JSON.parse(JSON.stringify(currentEvidences)));

                    if (testRunInfo.screenshots?.length > 0) {

                        for (const i in testRunInfo.screenshots) {
                ...
                if (hasWarnings) {
                    if (typeof currentTest.evidence === 'undefined')
                        currentTest.evidence = [];
                    ...
                    const buff = new Buffer(warningData);
                    currentEvidences.data = buff.toString('base64');
                    currentEvidences.filename = 'warning-log.txt';
                    currentEvidences.contentType = 'text/plain';
                    currentTest.evidence.push(JSON.parse(JSON.stringify(currentEvidences)));
                }
            }
            ...
            currentTest.comment = `Browsers: ${this.uaList}`;
